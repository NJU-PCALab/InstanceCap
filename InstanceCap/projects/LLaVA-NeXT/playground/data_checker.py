...
class DataProcessor:
    ...
    def check_image_existence(self, data):
        ...
        # if videos["conversations"][0]["value"].count("<image>") > 1:
        #     print(f"WARNING!!! {videos['conversations'][0]['value']} has more than one <image> !!!")
    ...
    def count_items(self):
        if isinstance(self.data, list):  # Assuming JSON videos loaded directly
            return len(self.data)
        elif isinstance(self.data, dict):  # Assuming YAML videos loaded
            total_items_count = 0
            for d in self.data["datasets"]:
                dd_json_path = d["json_path"]
                data = self.load_json_data(dd_json_path)
        ...
        if isinstance(self.data, dict):
            for d in self.data["datasets"]:
                ...
                if len(filtered_data) < len(data):
                    ...
                    print(f"Filtered videos count: {len(filtered_data)}")
                else:
                    pass
    ...
    def stat_and_filter_data(self, threshold):
        if isinstance(self.data, dict):
            ...
            for d in self.data["datasets"]:
                ...
                # Save filtered videos for each dataset
                if filtered_data and save_flag:
                    if dd_json_path.endswith(".jsonl"):
                        ...
                        with open(output_file, "w") as f:
                            json.dump(filtered_data, f, indent=2)
                    print(f"Filtered videos for {dd_json_path} saved to: {output_file}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_root", type=str, default="/mnt/bn/vl-research/videos/llava_data")
    parser.add_argument("--video_root", type=str, default="/mnt/bn/vl-research/videos/llava_video")
