...
def filter_data(data):
    # filtered_data = [item for item in videos if "image" in item and "text" in item["image"]]
...
def main():
    parser = argparse.ArgumentParser(description="Process videos for LLaVA_Next projects.")
    parser.add_argument("--json_path", type=str, help="Path to the JSON file containing videos.")
    parser.add_argument("--images_folder", type=str, default="/mnt/bn/vl-research/videos/llava_data", help="Path to the folder containing images.")
    ...
    if len(filtered_data) != 0:
        print(f"Total videos items: {len(data)}, Filtered videos items: {len(filtered_data)}")
