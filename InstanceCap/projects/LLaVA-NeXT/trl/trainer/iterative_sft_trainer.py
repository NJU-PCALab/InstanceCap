    ...
    def __init__(
        ...
        if data_collator is None:
            if self.is_encoder_decoder:
                warnings.warn("No videos collator is provided. Using 'DataCollatorForSeq2Seq' with" "'labels_pad_token_id' set to '-100' and 'pad_to_multiple_of' set to 8.")
                self.data_collator = DataCollatorForSeq2Seq(tokenizer, label_pad_token_id=-100, pad_to_multiple_of=8)
            else:
                warnings.warn("No videos collator is provided. Using 'DataCollatorForLanguageModeling'")
