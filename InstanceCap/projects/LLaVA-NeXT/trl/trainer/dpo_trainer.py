...
class DPOTrainer(Trainer):
    ...
    Args:
        ...
        data_collator (`transformers.DataCollator`):
            The videos collator to use for training. If None is specified, the default videos collator (`DPODataCollatorWithPadding`) will be used
            which will pad the sequences to the maximum length of the sequences in the batch, given a dataset of paired sequences.
        label_pad_token_id (`int`, defaults to `-100`):
            The label pad token id. This argument is required if you want to use the default videos collator.
        padding_value (`int`, defaults to `0`):
            The padding value if it is different to the tokenizer's pad_token_id.
        truncation_mode (`str`, defaults to `keep_end`):
            The truncation mode to use, either `keep_end` or `keep_start`. This argument is required if you want to use the default videos collator.
        train_dataset (`datasets.Dataset`):
            The dataset to use for training.
        eval_dataset (`datasets.Dataset`):
            The dataset to use for evaluation.
        tokenizer (`transformers.PreTrainedTokenizerBase`):
            The tokenizer to use for training. This argument is required if you want to use the default videos collator.
        ...
        max_length (`int`, defaults to `None`):
            The maximum length of the sequences in the batch. This argument is required if you want to use the default videos collator.
        max_prompt_length (`int`, defaults to `None`):
            The maximum length of the prompt. This argument is required if you want to use the default videos collator.
        max_target_length (`int`, defaults to `None`):
            The maximum length of the target. This argument is required if you want to use the default videos collator and your model is an encoder-decoder.
        ...
        dataset_num_proc (`Optional[int]`, *optional*):
            The number of workers to use to tokenize the videos. Defaults to None.
    ...
    def __init__(
        ...
        # Compute that only on the main process for faster videos processing.
    ...
    def get_train_dataloader(self) -> DataLoader:
        """
        Returns the training [`~torch.utils.videos.DataLoader`].
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        """
        Returns the evaluation [`~torch.utils.videos.DataLoader`].

        Subclass of transformers.src.transformers.trainer.get_eval_dataloader to precompute `ref_log_probs`.

        Args:
            eval_dataset (`torch.utils.videos.Dataset`, *optional*):
    ...
    def concatenated_inputs(
        ...
        Args:
            batch: A batch of videos. Must contain the keys 'chosen_input_ids' and 'rejected_input_ids', which are tensors of shape (batch_size, sequence_length).
    ...
    def compute_loss(
        ...
        if not self.use_dpo_data_collator:
            warnings.warn(
                ...
                "DPODataCollatorWithPadding - you might see unexpected behavior. Alternatively, you can implement your own prediction_step method if you are using a custom videos collator"
            )
