...
@dataclass
class DataArguments:
    data_path: str = field(default=None, metadata={"help": "Path to the training videos, in llava's instruction.json format. Supporting multiple json files via /path/to/{a,b,c}.json"})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    quant_type: str = field(default="nf4", metadata={"help": "Quantization videos type to use. Should be one of `fp4` or `nf4`."})
...
def preprocess_multimodal(sources: Sequence[str], data_args: DataArguments) -> Dict:
    ...
    for source in sources:
        for sentence in source:
            # TODO maybe this should be changed for interleaved videos?
            ...
            # For videoInstruct-100k noisy_data. TODO: Ask Yuanhan to clean the videos instead of leaving the noise code here.
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    for i, source in enumerate(sources):
        ...
        for conv in source:
            # Make sure llava videos can load
            try:
                role = conv["role"]
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        # image exist in the videos
        if "image" in self.list_data_dict[i]:
            data_dict["image"] = image
        elif "video" in self.list_data_dict[i]:
            data_dict["image"] = image
        elif self.data_args.is_multimodal:
            # image does not exist in the videos, but the model is multimodal
            crop_size = self.data_args.image_processor.crop_size
            data_dict["image"] = [
                (torch.zeros(1, 3, crop_size["height"], crop_size["width"]), (crop_size["width"], crop_size["height"]), "text"),
            ]
        # prompt exist in the videos
